...
object json_csGenerateResponse {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val proc_st_ts = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS").format(new java.util.Date())
      ...
      spark.sql(s"insert into table $sDBName.cs_logs select '$run_time_ts', 'cs', '004', 'Response Spark Job', 0, 'Response Process Started'")
      spark.sparkContext.setLogLevel("ERROR")


      //-------------006: cs_response_data--------------
      //val df_cs_response_data = spark.sql(s"SELECT * FROM $sDBName.cs_response_data WHERE request_id='$sRequestId' AND process_ts = '$proc_st_ts'")
      //val df_cs_response_data = spark.sql(s"SELECT * FROM $sDBName.cs_response_data WHERE request_id='$sRequestId' AND create_ts = (SELECT max(create_ts) FROM $sDBName.cs_response_data WHERE request_id='$sRequestId')") //.show(false)
      val df_cs_response_data = spark.sql(s"SELECT * FROM $sDBName.cs_response_data_stg")

      //if( json_cs_resp.select("results_req").collect.size >= 1) {
      if (df_cs_response_data.select("request_id").collect.size >= 1) {

        val json_cs_resp = df_cs_response_data.withColumn("statusCode", lit("SUCCESS")).withColumn("stdDevType", lit("SAMPLE")).
          ...
          )).select(concat(lit("cs_resp_"), $"request_id", lit("_"+new java.text.SimpleDateFormat("yyyy_MM_dd_HH_mm_ss").format(new java.util.Date())),lit(".json")).as("filename"), $"resp") //.show(false)

        val resp_km = json_cs_resp.map(rec => (rec.getString(0), rec.getString(1))).rdd.collectAsMap
        //val resp_km = json_cs_resp.rdd.map(r => (r.getString(0), r.getString(1))).collectAsMap()

        val sCurrDt = new java.text.SimpleDateFormat("yyyy_MM_dd_HH_mm_ss").format(new java.util.Date())
        for ((fileName, data) <- resp_km) {
          json_cs_resp.filter($"filename" === fileName).select("resp").coalesce(1).write.text(s"${opHDFSPath}/${sCurrDt}")
        ...
        json_cs_resp.select("filename").coalesce(1).write.text(s"${opHDFSPath}/${sCurrDt}")
        s"hdfs dfs -mv ${opHDFSPath}/${sCurrDt}/*.txt ${opHDFSPath}/cs_resp_${sCurrDt}.tag".!
        println(getDTFormat() + s": --> Outgoing tag file: ${opHDFSPath}/cs_resp_${sCurrDt}.tag")
        s"hdfs dfs -rm -R $opHDFSPath/$sCurrDt".!

    //val sCurrDt = new java.text.SimpleDateFormat("yyyy_MM_dd_HH_mm_ss").format(new java.util.Date())
    //json_cs_resp.coalesce(1).write.text(s"${opHDFSPath}/${sCurrDt}")
    //s"hdfs dfs -mv ${opHDFSPath}/${sCurrDt}/*.txt ${opHDFSPath}/${opJsonFile}".!
    //s"hdfs dfs -rm -R $opHDFSPath/$sCurrDt".!
        println(getDTFormat() + s": --> ${sDBName}.cs_response_data")
        spark.sql(s"insert into ${sDBName}.cs_response_data select * from ${sDBName}.cs_response_data_stg")

        run_time_ts = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS").format(new java.util.Date())
        spark.sql(s"insert into table ${sDBName}.cs_logs select '${run_time_ts}','cs','004','cs_response_data',0,'${opHDFSPath}/'")
        //spark.sql(s"truncate table ${sDBName}.cs_response_data_stg")

  } else {
    run_time_ts = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS").format(new java.util.Date())
    spark.sql(s"insert into table ${sDBName}.cs_logs select '${run_time_ts}','cs','004','cs_response_data',1,'No output data to generate output'")
  }

    spark.sql(s"refresh table ${sDBName}.cs_logs")
    spark.sql(s"refresh table ${sDBName}.cs_response_data")
    spark.sql(s"refresh table ${sDBName}.cs_response_data_stg")
